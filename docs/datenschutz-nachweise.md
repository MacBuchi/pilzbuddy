...
2. **Zusammenstellen**, im Supabase-Dashboard per SQL über die
   ...
   `find-photos` unter `<user_id>/`), `push_devices`, `feedback` (dazu
   Bilder im Bucket `feedback-photos` unter `<user_id>/`),
   `error_reports`.
...
| Nachrichten an Buddys | Absender, Empfänger, Text (≤ 500 Zeichen), Zeitpunkt, gelesen-Zeitpunkt | Art. 6 (1) b | Supabase, sichtbar nur für die beiden Beteiligten; nicht Ende-zu-Ende-verschlüsselt, für den Betreiber technisch lesbar | 30 Tage; sofort gelöscht bei Ablehnen, Zurückziehen oder Entfernen der Freundschaft, einzelne Nachrichten jederzeit durch den Absender |
| Aliase für Buddys (#567, Patch 032) | Besitzer, Buddy, Alias (≤ 40 Zeichen), Änderungszeitpunkt | Art. 6 (1) b | Supabase, sichtbar nur für den Besitzer (`fa_*`-Policies, anon ohne Grant); der Buddy erfährt nichts | bis zum Entfernen; sofort gelöscht, wenn die Freundschaft endet (Trigger `friendships_delete_aliases`), und per Cascade mit einem der beiden Konten |
| Benachrichtigung über eine Nachricht | Absendername, Text (≤ 180 Zeichen), Gerätekennung (Token) des Empfängers | Art. 6 (1) a (Einschalten der Benachrichtigungen) | Google (FCM) als Auftragsverarbeiter der Zustellung | nicht bei uns gespeichert; die Warteschlange hält die Nachrichten-id höchstens bis zum nächsten Minutenlauf |
| An iNaturalist melden | bei uns: Fund-Bezug, Kennung und Status der Beobachtung; an iNaturalist: Art, Datum, genaue Fundstelle mit Genauigkeit, Anzahl, gewählte Bäume, gewählte Fotos (ohne Metadaten) | Art. 6 (1) a | Supabase (nur der Nutzer selbst); iNaturalist als eigener Verantwortlicher, öffentlich (Stelle auf Wunsch verschleiert), weiter an GBIF | bei uns bis zur Löschung von Fund oder Konto; bei iNaturalist bis der Nutzer dort löscht |
