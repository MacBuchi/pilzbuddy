...
| **Nachrichten → Andere In-App-Nachrichten** | Ja | Nein¹ | Optional | App-Funktionalität | Nachrichten zwischen Buddys (#564, Patch 030): Text ≤ 500 Zeichen, Absender, Empfänger, Zeitpunkt, gelesen-Zeitpunkt (`buddy_messages`). Nur zwischen zwei Nutzern mit angenommener Freundschaft oder offener Anfrage (dann höchstens drei je Seite); sichtbar nur für die beiden. 30 Tage (Default + Check + Spalten-Grants, Cron räumt), sofort gelöscht bei Ablehnen/Entfernen. Nicht Ende-zu-Ende-verschlüsselt — so in der Datenschutzerklärung. *Geteilt*: nein, dieselbe Begründung wie „Freunde sehen meine Spots" (¹). Seit Patch 031 (1.194.0) geht der Text zusätzlich als Benachrichtigung über Google/FCM (Absendername + Text, ≤ 180 Zeichen), wenn der Empfänger Benachrichtigungen eingeschaltet hat — Google als Dienstleister für die Zustellung, dieselbe Einordnung wie bei Brevo (³), also weiter *nicht geteilt*; in der Datenschutzerklärung ausdrücklich benannt |
| **App-Aktivität → Andere nutzergenerierte Inhalte** | Ja | **Ja²** | Optional | App-Funktionalität, Entwicklerkommunikation | Spot-Name, Art, Notiz (`spots`, `finds`) sowie Feedback-Text und App-Version (`feedback`). Dazu die Aliase für Buddys (#567, Patch 032, seit 1.195.0): ein selbst vergebener Name je Buddy (`friend_aliases`), sichtbar nur für den, der ihn vergibt, gelöscht mit dem Ende der Freundschaft — dieser Teil ist *nicht geteilt* |
| **Fotos und Videos → Fotos** | Ja | **Ja⁵** | Optional | App-Funktionalität, Entwicklerkommunikation | Fundfotos (#532, seit 1.185.0): ein Bild, das der Nutzer ausdrücklich zu einem eigenen Fund teilt. Dazu seit 1.186.0 (#525) optional ein Bild an einer Feedback-Meldung (seit 1.196.0 bis zu drei, #569/Patch 033; seit 1.197.0 mit freiwilliger Einwilligung, ein Bild nach Ansicht unter CC BY-SA 4.0 mit dem Benutzernamen in die Artgalerie zu übernehmen — Patch 034, *geteilt* wird dann das Bild samt Name als Teil der App) — Bucket `feedback-photos`, ohne Leserecht für Nutzer, nur der Betreiber sieht es, der Bot löscht es nach 90 Tagen; anders als der Text wird es NICHT veröffentlicht. Vor dem Hochladen auf 1024 px verkleinert und von ALLEN Metadaten befreit (`lib/core/photo_pipeline.dart` — geprüft an den Bytes, weil `image_picker` beim Verkleinern die GPS-Tags zurückkopiert). Liegt im Supabase-Bucket `find-photos`, sichtbar nur für Buddys, die den Fund sehen dürfen (Policy erbt von `finds`), und läuft nach 14 Tagen ab (`find_photos.expires_at`, Bot räumt Zeilen und Objekte). Keine Kamera-Berechtigung: Kamera-Intent und Photo Picker kommen ohne aus. Seit #553 auf ausdrücklichen Wunsch Fotos eines Fundes an iNaturalist, siehe ⁵ |
